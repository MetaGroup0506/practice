...
public class Practice1 {
    public static void main(String[] args) {
        ...
        System.out.println("[요구사항 1] 데이터를 List에 추가");
        System.out.println(getStringBuilder(customers));

        // [요구사항 2] 다음 데이터를 List의 맨 앞에 추가
        customers.add(0, new Customer("이황", 66, "남", 9999.9));
        System.out.println("[요구사항 2] 다음 데이터를 List의 맨 앞에 추가");
        System.out.println(getStringBuilder(customers));

        // [요구사항 3] 고객 이름을 가나다순으로 정렬
        customers.sort(Comparator.comparing(Customer::getName));
        System.out.println("[요구사항 3] 고객 이름을 가나다순으로 정렬");
        System.out.println(getStringBuilder(customers));

        // [요구사항 4] 고객 나이를 오름차순으로 정렬
        customers.sort(Comparator.comparing(Customer::getAge));
        System.out.println("[요구사항 4] 고객 나이를 오름차순으로 정렬");
        System.out.println(getStringBuilder(customers));
    }

    private static StringBuilder getStringBuilder(List<Customer> customers) {
        ...
        for (Customer customer : customers) {
            printCustomer.append(String.format("%s | %d | %s | %.1f\n",
                            customer.getName(), customer.getAge(),
                            customer.getGender(), customer.getPoint()
            ));
        }
